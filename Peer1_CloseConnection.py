...
import socket
import threading
from queue import Queue # to implement sequence numbers
import time # to implement timeout & ACKs

# Initializing variables to be used as Global Variables

my_sequence_number = 0 # peer 1 sequence number
other_sequence_number = 0 # peer 2 expected sequence number
sending_ID = 1 # peer 1 message ID 
receiving_ID = 1 # peer 2 expected message ID
...
messaging = 1 # flag that indicates peer 1 is sending a message (when it is asserted)
receiving = 1 # flag that indicates peer 1 is receiving a message (when it is asserted)
...
incoming_queue = Queue() # used to store received packets from peer 2
...
def send_message(user_UDP_socket, client_address, user_TCP_socket):
    ...
    while True: # infinite loop
       ...
       if not stop_threading: # while threading is not stopped
            ...
            messaging = 1 # set messaging flag to 1 because peer 1 is sending a message 
            ...
            elif (message == "") and (stop_threading): # if peer 1 pressed enter and the threading must stop
    ...
    while True: # infinite loop
        ...
        if not stop_threading: # while threading is not stopped
            ...
            if not incoming_queue.empty(): # a response was received & saved in incoming_queue
                ...
                receiving = 1 # set receiving flag to 1 because I am receiving/processing packets -> peer 1 is not idle
                ...
                receiving = 0 # set receiving flag to 0 because incoming_queue is empty <=> peer 1 is not receiving packets <=> peer 1 is idle
    ...
    while True: # infinite loop
        ...
        if not stop_threading: # while threading is not stopped
            ...
            if len(sent_packets) != 0:
                ...
                messaging = 1 # set messaging flag to 1 because I am awaiting an ACK or else sending back a missing packet => peer 1 is not idle!
...
def check_for_connection():
    ...
    counter = 6 # used as a countdown to alert peer 1 before closing the connection
    ...
    while True: # infinite loop
        ...
        if (time.time() - idle_time >= 30) and (not receiving) and (not messaging): # idle for 30 seconds and not messaging nor receiving
            ...
            if counter > -1: # to avoid errors
                ...
                print("Your chat connection will close in " + str(counter) + " seconds") # alerting peer 1
...
def send_TCP_file(client_address, file_and_extension_name):
    ...
    if not stop_threading:
        ...
        sending_socket.connect(client_address) # connect to peer 2 using TCP
        ...
        sending_socket.send(str(file_name_to_client).encode()) # peer 2 gets file name
        print("Sending...")
        data = file.read()
        sending_socket.sendall(data) # sends all file data to peer 2
